...
vec4 SmapleColor(int index, vec2 uv){
if(index == 0) {
...
void main() {
    ...
    // Ensure UV coordinates are within texture bounds
    if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) {
        discard;
    }
     vec4 startTexture = SmapleColor(uStartIndex, uv);
    vec4 endTexture = SmapleColor(uEndIndex, uv);
